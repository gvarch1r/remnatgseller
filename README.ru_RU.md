...
> [!WARNING]
> **Последняя версия бота совместима с панелью RemnaWave версии 2.3.x–2.6.x**  
> Для панели 2.6.x используйте обновлённую зависимость remnapy (см. [Совместимость с Remnawave 2.6.x](#remnawave-26x-совместимость) ниже).

### Remnawave 2.6.x совместимость

Для панели Remnawave **2.6.x** в репозитории включена папка `remnapy-production` с обновлениями API (customRemarks: `emptyInternalSquads` удалён, добавлены `HWIDNotSupported` и `HWIDMaxDevicesExceeded`). Зависимость remnapy по пути настроена по умолчанию в `pyproject.toml`.
...
Для корректной работы RemnatgSeller требуется настроенный обратный прокси.  
...
При использовании `RESET_ASSETS=true` выполняются следующие действия:  
  - Все текущие ассеты сохраняются в резервные копии с отметкой времени (`/opt/remnatgseller/assets/*.bak`).  
...
Разместите баннеры по пути: `/opt/remnatgseller/assets/banners/(locale)/`  
Пример: `/opt/remnatgseller/assets/banners/en/menu.gif`
