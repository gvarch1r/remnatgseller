...
from src.application.dto import DeviceAddonDto, PlanDto, PriceDetailsDto, UserDto
...
async def subscription_getter(
    ...
    **kwargs: Any,
    ...
    has_devices_limit = (
        current_subscription.has_devices_limit if current_subscription else False
    )
...
async def plans_getter(
    ...
    **kwargs: Any,
    ...
    if "purchase_type" in start_data:
        ...
        if pt == PurchaseType.RENEW and current is not None and current.is_trial:
            pt = PurchaseType.NEW
    ...
    **kwargs: Any,
    ...
    if purchase_type == PurchaseType.ADD_DEVICES:
        ...
        for gateway in gateways:
            ap = addon.get_price(gateway.currency)
            if ap is not None:
        ...
        dk, dw = i18n_format_device_limit(addon.device_count)
        devices_str = i18n.get(dk, **dw)
        return {
...
async def success_payment_getter(
    ...
    **kwargs: Any,
    ...
    if purchase_type == PurchaseType.ADD_DEVICES:
        device_count = int(start_data.get("device_count", 0))
        dk, dw = i18n_format_device_limit(device_count)
        added_duration = i18n.get(dk, **dw)
    else:
