...
async def _quick_extend_days(
    dialog_manager: DialogManager,
    notifier: Notifier,
    add_subscription_duration: Any,
    days: int,
) -> None:
    user: UserDto = dialog_manager.middleware_data[USER_KEY]
    target_telegram_id = dialog_manager.dialog_data[TARGET_TELEGRAM_ID]
    try:
        await add_subscription_duration(
            user,
            AddSubscriptionDurationDto(target_telegram_id, days),
        )
    except ValueError:
        await notifier.notify_user(
            user=user,
            payload=MessagePayloadDto(
                ...
                i18n_kwargs={"operation": "ADD"},
            ),
        )


@inject
async def on_quick_extend_7(
    callback: CallbackQuery,
    widget: Button,
    dialog_manager: DialogManager,
    notifier: FromDishka[Notifier],
    ...
    await _quick_extend_days(dialog_manager, notifier, add_subscription_duration, 7)


@inject
async def on_quick_extend_30(
    callback: CallbackQuery,
    widget: Button,
    dialog_manager: DialogManager,
    notifier: FromDishka[Notifier],
    ...
    await _quick_extend_days(dialog_manager, notifier, add_subscription_duration, 30)
