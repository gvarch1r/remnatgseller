        ...
        async with self.uow:
            try:
                ...
                # 3. CHANGE OR CONVERT FROM TRIAL
                    ...
                    if not subscription:
                        raise ValueError(
                            f"No subscription found for add-devices for user '{user.telegram_id}'"
                        )
                    if subscription.is_trial:
                        raise ValueError(
                            f"Add-devices not allowed on trial for user '{user.telegram_id}'"
                        )

                    addon_slots = plan.device_limit
                    subscription.device_limit = subscription.device_limit + addon_slots
                    ...
                    await self.uow.commit()
                    logger.debug(
                        f"{actor.log} Added '{addon_slots}' device slots for user '{user.telegram_id}'"
                    )

                    await self.redirect.to_success_payment(
                        user.telegram_id,
                        purchase_type,
                        device_count=addon_slots,
                    )
                else:
                    raise ValueError(
