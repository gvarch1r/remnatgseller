...
from src.bot.states import RemnatgsellerPlans
...
async def on_plan_select(
    ...
    await sub_manager.switch_to(state=RemnatgsellerPlans.CONFIGURATOR)
...
async def on_plan_delete(
    ...
    if is_double_click(dialog_manager, key=f"delete_confirm_{plan.id}", cooldown=10):
        ...
        await dialog_manager.start(state=RemnatgsellerPlans.MAIN, mode=StartMode.RESET_STACK)
        return
...
async def on_name_input(
    ...
    await dialog_manager.switch_to(state=RemnatgsellerPlans.CONFIGURATOR)
...
async def on_duration_select(
    ...
    await sub_manager.switch_to(state=RemnatgsellerPlans.PRICES)
...
async def on_duration_input(
    ...
    await dialog_manager.switch_to(state=RemnatgsellerPlans.DURATIONS)
...
async def on_currency_select(
    ...
    await dialog_manager.switch_to(state=RemnatgsellerPlans.PRICE)
...
async def on_price_input(
    ...
    adapter.save(plan)
    await dialog_manager.switch_to(state=RemnatgsellerPlans.PRICES)
...
async def on_squads(
    ...
    await dialog_manager.switch_to(state=RemnatgsellerPlans.SQUADS)
...
async def on_confirm_plan(  # noqa: C901
    ...
    await dialog_manager.start(state=RemnatgsellerPlans.MAIN)
