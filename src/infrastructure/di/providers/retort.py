...
from src.core.enums import MediaType, PaymentGatewayType, ReferralLevel, Role, YookassaVatCode
...
class RetortProvider(Provider):
    ...
    def get_conversion_retort(
        ...
        def get_settings_dto(pg_type: PaymentGatewayType, settings_dict: dict) -> Any:
            ...
            if dto_class is YookassaGatewaySettingsDto:
                vc = settings_dict.get("vat_code")
                if vc is not None and not isinstance(vc, YookassaVatCode):
                    settings_dict["vat_code"] = YookassaVatCode(int(str(vc).strip()))
        ...
        conversion_retort = ConversionRetort(
            recipe=[
                ...
                coercer(
                    dict, CurrencyRatesSettingsDto, retort.get_loader(CurrencyRatesSettingsDto)
                ),
