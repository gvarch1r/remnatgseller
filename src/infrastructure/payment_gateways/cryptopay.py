...
class CryptoPayGateway(BasePaymentGateway):
    ...
    async def handle_webhook(self, request: Request) -> tuple[UUID, TransactionStatus]:
        ...
        invoice: dict = webhook_data.get("payload") or {}

        if update_type == "invoice_paid":
            payload_str = invoice.get("payload")
            if not payload_str:
                raise ValueError("Required field 'payload' (order id) is missing in invoice")

            status = invoice.get("status")
            payment_id = UUID(str(payload_str))
            ...
            match status:
                ...
                case _:
                    logger.debug(
                        f"CryptoPay invoice_paid with non-final status '{status}', ignoring"
                    )
        ...
        if update_type == "invoice_expired":
            payload_str = invoice.get("payload")
            if payload_str:
                return UUID(str(payload_str)), TransactionStatus.CANCELED
            raise PaymentWebhookIgnored()

        logger.debug(f"CryptoPay webhook update_type '{update_type}' ignored")
        raise PaymentWebhookIgnored()
