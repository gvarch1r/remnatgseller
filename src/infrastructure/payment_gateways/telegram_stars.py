...
class TelegramStarsGateway(BasePaymentGateway):
    ...
    async def handle_webhook(self, request: Request) -> tuple[UUID, TransactionStatus]:
        raise NotImplementedError(
            "Telegram Stars payments are received via the main Telegram webhook "
            "(successful_payment in Update), not via the payments webhook. "
            "This endpoint should not be called for Telegram Stars."
        )
