...
class SettingsService(BaseService):
    ...
    def _propagate_nested_changes(self, settings: SettingsDto) -> None:
        """Ensure nested TrackableDto changes are reflected in parent's changed_data."""
        if settings.user_notifications.changed_data:
            setattr(settings, "user_notifications", settings.user_notifications)
        if settings.system_notifications.changed_data:
            setattr(settings, "system_notifications", settings.system_notifications)
        if settings.referral.changed_data:
            setattr(settings, "referral", settings.referral)

    async def update(self, settings: SettingsDto) -> SettingsDto:
        self._propagate_nested_changes(settings)
