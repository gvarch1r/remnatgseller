...
class UserService(BaseService):
    ...
    async def search_users(self, message: Message) -> list[UserDto]:
        ...
        if message.forward_from and not message.forward_from.is_bot:
            ...
            if search_query.isdigit():
            ...
            elif search_query.startswith(REMNATGSELLER_PREFIX):
                try:
                    target_id = int(search_query.split("_", maxsplit=1)[1])
                    single_user = await self.get(telegram_id=target_id)
                    if single_user:
                        found_users.append(single_user)
                        logger.info(f"Searched by RemnatgSeller ID '{target_id}', user found")
                    else:
                        logger.warning(f"Searched by RemnatgSeller ID '{target_id}', user not found")
                ...
                if self.remnawave and search_query.replace("_", "").replace("-", "").isalnum():
                    try:
                        ...
                        if remna_user and remna_user.telegram_id:
                            single_user = await self.get(
                                telegram_id=remna_user.telegram_id
                            )
                            if single_user:
                                found_users.append(single_user)
                                logger.info(
                                    f"Searched by panel username '{search_query}', "
                                    "user found"
                                )
                        ...
                        logger.debug(
                            f"Panel username search failed for '{search_query}': {exc}"
                        )
                if not found_users:
