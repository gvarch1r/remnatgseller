...
The following file formats are supported, as defined in `/remnatgseller/src/core/enums.py` as `BannerFormat`:
...
Banner filenames must correspond to the following predefined names, specified in `/remnatgseller/src/core/enums.py` as `BannerName`:
...
* **`APP_LOCALES`**: A list of supported locales. A full list of available locales can be found in `remnatgseller/src/core/enums.py` as `Locale`.
