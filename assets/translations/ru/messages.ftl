# Menu
msg-main-menu =
    <b>Быстрый старт:</b> «Подключение» — доступ · «Подписка» — оплата · «Поддержка» — помощь.
...
msg-menu-devices =
    ...
    *[HAS] Нажмите на устройство, чтобы удалить его.
    Нужно больше слотов — кнопка «Докупить устройства» или смена тарифа в «Подписка».
    }
...
msg-device-addons-main =
    <b>📱 Докупка устройств</b>

    Добавляйте пакеты кнопкой ниже. В списке можно включать и отключать пакеты.

msg-device-addons-empty = <b>📱 Докупка устройств</b>

    <i>Пакетов пока нет. Нажмите «Добавить пакет» или выполните миграции БД (сид в миграции).</i>

msg-device-addons-add-count =
    <b>📱 Новый пакет</b>

    Сколько <b>дополнительных устройств</b> даёт этот пакет? Отправьте одно целое число (например <code>1</code> или <code>2</code>).

msg-device-addons-add-prices =
    <b>📱 Цены пакета</b>

    Устройств в пакете: <b>+{ $addon_device_count }</b>

    Отправьте по одной строке на валюту: <code>RUB 199</code>, <code>XTR 50</code>, <code>USD 2</code>. Дубликаты валют недопустимы.
...
msg-subscription-add-devices =
    ...
    Выберите, сколько дополнительных слотов устройств добавить к текущей подписке.
msg-subscription-add-devices-empty = <b>📱 Докупка устройств</b>

    Пакеты докупки сейчас недоступны. Попробуйте позже.
msg-subscription-promocode =
    <b>🎟 Активация промокода</b>

    Введите код сообщением ниже. Скидка применится к следующим покупкам; промокод на подписку выдаст доступ сразу.
...
msg-subscription-confirm =
    <b>{ $purchase_type ->
    [ADD_DEVICES] 🛒 Подтверждение докупки устройств
    [RENEW] 🛒 Подтверждение продления подписки
    [CHANGE] 🛒 Подтверждение изменения подписки
    *[OTHER] 🛒 Подтверждение покупки подписки
    }</b>

    { msg-subscription-details }

    { $purchase_type ->
    [ADD_DEVICES] <i>⚠️ К лимиту устройств вашей подписки будут добавлены выбранные слоты.</i>
    [RENEW] <i>⚠️ Текущая подписка будет <u>продлена</u> на выбранный срок.</i>
...
msg-subscription-add-devices-success = К подписке добавлено: { $added_duration }.
...
msg-promocode-code = <b>🏷️ Код промокода</b>
    ...
    Отправьте код одним сообщением (латиница и цифры).
...
msg-promocode-reward-numeric-hint =
    Введите целое число: дни, ГБ трафика или процент скидки — в зависимости от типа.

msg-promocode-reward-subscription =
    Нажмите «Выбрать план», затем укажите план и срок (если у плана несколько вариантов).
...
msg-promocode-plan-duration-missing =
    Нет вариантов срока для этого плана.

msg-promocode-lifetime = <b>⌛ Срок действия промокода</b>

    Введите срок в <b>днях</b>. <b>-1</b> — без ограничения по времени.

msg-promocode-activations = <b>🔢 Лимит активаций</b>

    <b>-1</b> — без лимита, иначе целое число ≥ 0.

msg-promocode-delete-confirm =
    <b>🗑 Удаление промокода</b>

    Удалить промокод <code>{ $delete_promocode_code }</code>? Связанные активации будут удалены.