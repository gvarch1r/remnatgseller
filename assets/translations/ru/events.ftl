...
event-subscription =
    ...
    .add-devices =
    #SubscriptionAddDevicesEvent

    <b>🔅 Событие: Докупка слотов устройств!</b>
    ...
    { $is_trial ->
    [0]
    <b>⚠️ Подписка закончится через { unit-day }.</b>

    Продлите заранее: <b>меню → «Подписка» → Продлить</b>.
    *[1]
    <b>⚠️ Пробный период закончится через { unit-day }.</b>

    Оформите подписку: <b>меню → «Подписка»</b>.
    }
    ...
    *[1] Ваш бесплатный пробный период закончился { unit-day } назад. Оформите подписку, чтобы продолжить пользоваться сервисом!
    ...
    .limited =
    <b>⛔ Доступ приостановлен — трафик израсходован.</b>
...
event-remnashop-welcome =
    ...
    ⭐ <i>Поставьте звездочку на <a href="{ $repository }">GitHub</a> и следите за <a href="{ $releases_url }">релизами</a>.</i>